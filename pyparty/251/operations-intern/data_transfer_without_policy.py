...
class Node:
    ...
    # Local transfer without Master validation
    def update_inventory(self, data):
        # Update local inventory (reduce stock at source node)
        for item, quantity in data.items():
            self.inventory[item] = max(0, self.inventory.get(item, 0) - quantity)  # Ensure stock doesn't go negative
...
async def main():
    ...
    while True:
        ...
        if data_to_transfer:
            ...
            source_node.update_inventory(data_to_transfer)
